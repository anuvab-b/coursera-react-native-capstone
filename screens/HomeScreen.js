...
const HomeScreen = ({ navigation }) => {
  ...
  const loadMenuData = async (database) => {
    try {
      const result = await getAllMenuItems(database);
  ...
  const fetchAndStoreMenuData = async (database) => {
    try {
      ...
      if (data.menu && Array.isArray(data.menu)) {
        await saveMenuItems(database, data.menu);

        const storedItems = await getAllMenuItems(database);
      ...
      Alert.alert(
        "Error",
        "Failed to load menu data. Please check your internet connection."
      );
    }
  };
  ...
  const loadCategories = async (database) => {
    try {
      const categoryList = await getCategories(database);
  ...
  const handleFilter = async () => {
    ...
    try {
      const filtered = await filterBySearchAndCategories(
        db,
        debouncedSearchText,
        selectedCategories
      );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Banner with Search */}
      <Banner searchText={searchText} onSearchChange={setSearchText} />
